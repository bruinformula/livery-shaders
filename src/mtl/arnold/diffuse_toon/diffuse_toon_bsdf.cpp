...
bsdf_init
{
    DiffuseToonBSDF* data = (DiffuseToonBSDF*)AiBSDFGetData(bsdf);
...
bsdf_sample
{
    ...
    float cosNI, pdf;
    const AtVector wi = sample_uniform_cone(data->N, omc, rnd.x, rnd.y, cosNI, pdf);

    if (!(AiV3Dot(wi, data->Ng) > 0.0f))
        return AI_BSDF_LOBE_MASK_NONE;

    const float angle = acosf(AiMax(cosNI, 0.0f));
...
AtBSDF* DiffuseToonBSDFCreate(const AtShaderGlobals* sg,
                               ...
                               float smooth)
{
    AtBSDF* bsdf = AiBSDF(sg, color, DiffuseToonBSDFMtd, sizeof(DiffuseToonBSDF));
    DiffuseToonBSDF* data = (DiffuseToonBSDF*)AiBSDFGetData(bsdf);
    data->N = N;
    data->size = size;
    data->smooth = smooth;
